...
def find_centroid(cluster):
    ...
    lat = mean([pair[0] for pair in locations])
    lon = mean([pair[1] for pair in locations])
    return [lat,lon]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_y = mean(ys)
    xy_list = zip(xs,ys)
    s_xx = sum([(x - mean_x)**2 for x,y in xy_list])
    s_yy = sum([(y - mean_y)**2 for x,y in xy_list])
    s_xy = sum([(x - mean_x)*(y - mean_y) for x,y in xy_list])
    b = s_xy / s_xx
    a = mean_y - b * mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor, r2 = max([find_predictor(user, reviewed, fn) for fn in feature_fns], key=lambda elem:elem[1])
    return predictor
    # END Question 8
